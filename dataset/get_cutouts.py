...
for i,((root_data,dirs_data,files_data),(root_seg,dirs_seg,files_seg)) in enumerate(zip(sorted(os.walk(os.path.join(path,'images'))),sorted(os.walk(os.path.join(path_seg,'segmentations'))))):
    print(root_data,root_seg)
    if i<=0: # skip 'images/'
        continue
    print(root_data[root_data.find('\\')+1:])
    new_dir = os.path.join(path,'cutouts',root_data[root_data.find('\\')+1:])
    os.mkdir(new_dir)
